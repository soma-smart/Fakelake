...
fn extract_array(column_name: &str, array: &[Yaml], default_value: &str) -> Vec<WStringParameter> {
    array
        .iter()
        .map(|i| match i {
            Yaml::Hash(h) => extrat_hash(column_name, h, default_value),
...
fn extrat_hash(
    ...
    let weight = hash
        .get(&Yaml::String(String::from(WEIGHT_KEY)))
        .and_then(|w| w.as_i64());
    hash.get(&Yaml::String(String::from(VALUE_KEY)))
        .map(|f| match f {
